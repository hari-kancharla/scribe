...
enum AppWindowLayout {
    /// Opening size. The window is free to grow from here — these are not caps.
    static let width: CGFloat = 950
    static let minimumHeight: CGFloat = 750
    /// Below this the sidebar and content stop coexisting sensibly.
...
class WindowManager: NSObject {
    ...
    private static let mainWindowIdentifier = NSUserInterfaceItemIdentifier("com.personal.scribe.mainWindow")
    private static let mainWindowAutosaveName = NSWindow.FrameAutosaveName("ScribeMainWindowFrame")
    ...
    private func enforceMainWindowFrameIfNeeded(on window: NSWindow, preserveRestoredOrigin: Bool) {
        ...
        guard currentFrame.width < AppWindowLayout.minimumWidth
            || currentFrame.height < AppWindowLayout.minimumHeight
        else {
            return
        }
