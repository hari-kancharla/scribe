...
actor ScribeRefineXPCClient {
                        ...
                        reply.resolve(
                            .failure(
                                makeScribeRefineXPCError(
                                    .connectionFailed,
                                    description: "Could not create the Scribe Refine XPC proxy."
                                )
                            )
                        )
                        return
        ...
        do {
                            ...
                            reply.resolve(
                                .failure(
                                    makeScribeRefineXPCError(
                                        .invalidResponse,
                                        description:
                                            "Scribe Refine returned an empty response."
                                    )
                                )
                            )
                        }
            ...
            guard response.requestID == request.requestID else {
                throw makeScribeRefineXPCError(
                    .invalidResponse,
                    description: "Scribe Refine returned a mismatched response."
                )
            }
    ...
    private func localizedError(
        ...
        switch code {
        case .invalidRequest:
            description = String(localized: "Scribe Refine received an invalid request.")
        case .inferenceFailed:
            switch operation {
            case .prepare:
                description = String(localized: "Scribe Refine could not prepare the model.")
            case .enhance:
                description = String(
                    localized: "Scribe Refine could not complete the enhancement."
                )
            }
        case .invalidResponse:
            description = String(localized: "Scribe Refine returned an invalid response.")
        case .connectionFailed:
            description = String(localized: "Could not communicate with Scribe Refine.")
        }
    ...
    private func logFailure(
        ...
        logger.error(
            "Scribe Refine XPC \(operationName, privacy: .public) failed domain=\(nsError.domain, privacy: .public) code=\(nsError.code, privacy: .public) description=\(nsError.localizedDescription, privacy: .public)"
        )
    }
    ...
    private func connectionInterrupted(
        ...
        cancelIdleShutdown()
        logger.error(
            "Scribe Refine XPC connection interrupted id=\(identifier.uuidString, privacy: .public)"
        )
    }
