...
enum ScribeRefineAvailability: Equatable {
    ...
    /// No model repository is configured, so there is nothing to download.
    case noModelSource
}
...
enum ScribeRefineError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable:
            return String(localized: "Scribe Refine requires an Apple silicon Mac with at least 16 GB of memory.")
        case .modelNotDownloaded:
            return String(localized: "Scribe Refine is not downloaded.")
        }
    }
}
...
final class ScribeRefineService: ObservableObject {
    ...
    static let providerName = "Scribe Refine"
    static let modelName = "Scribe Refine V1"
    static let systemPrompt = """
        ...
        """
    /// No refine model is configured for this build.
    static let repositoryID = ""
    ...
    var unavailableDescription: String? {
        ...
        case .noModelSource:
            return String(localized: "No model repository is configured.")
        }
    }
    ...
    private init(
        ...
        } else if Self.repositoryID.isEmpty {
            availability = .noModelSource
        } else {
    ...
    func deleteModel() async {
        ...
        do {
            ...
            logger.error("Failed to delete Scribe Refine: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private func downloadModel() async {
        ...
        defer {
            ...
            do {
                ...
                downloadError = error.localizedDescription
                logger.error("Failed to download Scribe Refine: \(error.localizedDescription, privacy: .public)")
            }
        #else
