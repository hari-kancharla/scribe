...
struct ScribeApp: App {
    ...
    init() {
        ...
        let logger = Logger(subsystem: "com.personal.scribe", category: "Initialization")
        ...
        let appSupportDirectory = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("com.personal.scribe")
    ...
    private static func createPersistentContainer(schema: Schema, logger: Logger) throws -> ModelContainer {
        let appSupportURL = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("com.personal.scribe", isDirectory: true)
    ...
    var body: some Scene {
        ...
        // WindowManager owns the real constraints (resizable style mask, min size,
        // unbounded max). contentMinSize would ask SwiftUI to derive a size from
        // content that is deliberately unbounded, and it then creates no window.
        ...
        MenuBarExtra(isInserted: $showMenuBarIcon) {
            ...
            let image: NSImage = {
                // Scale by height so the mark's optical weight matches the system
                // glyphs beside it. 18pt rather than the menu bar's full 22pt because
                // SF Symbols carry internal padding — their ink is roughly 15pt at an
                // 18pt size — whereas this mark's ink fills its frame edge to edge.
                let targetHeight: CGFloat = 15
                let size = $0.size
                guard size.width > 0, size.height > 0 else { return $0 }
                let scale = targetHeight / size.height
                $0.size = NSSize(width: size.width * scale, height: size.height * scale)
                return $0
