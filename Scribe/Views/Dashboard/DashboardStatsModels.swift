...
enum DashboardInsightPeriod: String, CaseIterable, Identifiable, Sendable {
    ...
    var timeSavedContext: LocalizedStringKey {
        switch self {
        case .today: return "with Scribe today"
        case .lastSevenDays: return "with Scribe this week"
        case .lastThirtyDays: return "with Scribe over the last 30 days"
        case .thisYear: return "with Scribe this year"
        case .allTime: return "with Scribe"
        }
    }
...
struct DashboardStatsSummary: Codable, Equatable, Sendable {
    ...
    // Streaks are measured in whole calendar days that contain at least one session.
    var currentStreak: Int = 0
    var longestStreak: Int = 0
    var activeDayCount: Int = 0
    /// Whether a session has already been recorded today, so the UI can tell
    /// "streak extended today" from "streak alive but today is still open".
    var hasSessionToday: Bool = false
    /// One entry per calendar day across the contribution graph's window,
    /// oldest first, including days with no activity.
    var dailyActivity: [DashboardDayActivity] = []
}

/// A single cell in the contribution graph.
struct DashboardDayActivity: Codable, Equatable, Sendable, Identifiable {
    var date: Date
    var words: Int
    var sessions: Int
    /// 0 for an empty day, then 1...4 by volume, matching the four-step legend.
    var intensity: Int
    /// Marks the run of days making up the streak that is still running.
    var isInCurrentStreak: Bool
