...
class RecorderUIManager: ObservableObject, RecorderPanelPresenting {
    ...
    private let logger = Logger(subsystem: "com.personal.scribe", category: "RecorderUIManager")
    ...
    /// Call after ScribeEngine is created to break the circular init dependency.
    func configure(engine: ScribeEngine, recorder: Recorder) {
        self.engine = engine
